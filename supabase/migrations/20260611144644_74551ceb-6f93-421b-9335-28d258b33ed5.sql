-- Orders: restrict realtime to non-sensitive columns
ALTER PUBLICATION supabase_realtime DROP TABLE public.orders;
ALTER TABLE public.orders REPLICA IDENTITY DEFAULT;
ALTER PUBLICATION supabase_realtime ADD TABLE public.orders
  (id, user_id, status, currency, subtotal, shipping, tax, total, created_at, updated_at, discount, promo_code, payment_method, payment_status, fulfillment_status, tracking_number, carrier, stock_state, expires_at, market_region, payment_provider, is_seeded, attribution_session_id, paid_at, fulfilled_at, cancelled_at, cancel_window_expires_at);

-- Payments: restrict realtime to non-sensitive columns
...
-- Products: restrict realtime to non-sensitive columns (exclude cost, margins, revenue, admin_notes, warehouse_location, restock_eta)
