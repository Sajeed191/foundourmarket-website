-- 1. New product columns for badges, soft-delete, shipping, merchandising
...
-- 2. Secure, audited bulk product action RPC
...
BEGIN
  ...
  -- Snapshot relevant fields (capped) for audit before/after
  ...
  ELSIF _action = 'duplicate' THEN
    FOR r IN SELECT * FROM public.products WHERE id=ANY(_ids) LOOP
      ...
      INSERT INTO public.products (
        ...
        r.collection, r.homepage_section
      );
  ...
  ELSIF _action = 'set_sale' THEN
    -- params.pct: discount percent off, store original in compare_price
...
REVOKE ALL ON FUNCTION public.admin_bulk_products(uuid[], text, jsonb) FROM public, anon;
GRANT EXECUTE ON FUNCTION public.admin_bulk_products(uuid[], text, jsonb) TO authenticated, service_role;